<template>
  ...
  <div class="container">
    <h2>Les livreurs</h2>
    <v-btn @click="acceptPending" variant="outlined">Accept le livreur</v-btn>
    <EasyDataTable
...
export default {
  ...
  data: function (){
    return {
      ...
      excludedKey: ["img", "__v"],
      selected: [],
      headers: new Deliverer({_id:""}).headers(),
      singleSelect: false,
      itemsSelected: ref([])
    }
  },
    ...
    async acceptPending() {
      if (this.itemsSelected.length == 1) {
        if (this.itemsSelected[0].status == "pending"){
          const r = await new DelivererService().acceptPendingDeliverer({
          ...
          if(r.success){
            const result = await new DelivererService().get({request: 'getAll'}, this.$store, this.$router);
            if (result.success){
